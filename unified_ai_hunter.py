...
class UnifiedAIHunter:
    ...
    def execute_daily_routine(self):
        ...
        target = self.config.get('application_strategy', {}).get('daily_application_target', 30)
        
        for job in sorted(relevant_jobs, key=lambda x: x['relevance_score'], reverse=True):
            if applications_today >= target:
                break
            
            if job['relevance_score'] >= self.config.get('application_strategy', {}).get('auto_apply_threshold', 0.85):
